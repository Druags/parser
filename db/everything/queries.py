...
from db.everything.data_work import expand_manga_data
...
from db.everything.dataclasses import TableContent, TableRelation
...
def get_id(session: Session, orm_name: Type[Base], key_n_a_v: dict):
    """
    :param session:
    :param orm_name:
    :param key_n_a_v: takes dict {field_name: field_value}
    :return:
    """
    if pd.isnull(key_n_a_v):
        return
    try:
        return session.query(orm_name).filter_by(**key_n_a_v).first().id
    except AttributeError:
        session.add(orm_name(**key_n_a_v))
...
def add_categories(session_factory: sessionmaker) -> None:
    cat_data = [TableContent(TypeORM, data.title_types),
    ...
    for element in cat_data:
        df_to_orm(session_factory, orm_name=element.orm_name, converted_data=element.content)
